...
const GameSync = {
    ...
    init: function(supabaseClient) {
        // If no client provided, get from EnhancedSupabaseClient
        this.supabase = supabaseClient || EnhancedSupabaseClient.getInstance();
        ...
        console.log('GameSync initialized');
    },
    ...
    collectGameData: function() {
        ...
        const shardData = Utils.loadFromStorage('shards') || {};
        const equipmentData = {};
    ...
    applyGameData: function(gameData) {
        ...
        if (gameData.character_shards) {
            Utils.saveToStorage('shards', gameData.character_shards);
        }
        
        // Apply equipment
        if (gameData.equipment && typeof Inventory !== 'undefined') {
            // For each character, apply equipment
            Object.entries(gameData.equipment).forEach(([characterId, equipment]) => {
                Inventory.setCharacterEquipment(characterId, equipment);
            });
        }
